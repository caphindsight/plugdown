...
typedef struct {
  char *search_before, *search_after;
  char **cmd;
} Plugin;
...
static int doplugin(const char *begin, const char *end, int newblock);  /* Parser for plugin invokation */
...
static Parser parsers[] = { dounderline, docomment, dolineprefix,
                            dolist, doparagraph, dogtlt, doplugin,
                            dosurround, dolink, doshortlink, dohtml,
                            doamp, doreplace };
static int nohtml = 0;
...
static char* plugin_sort[] = {"sort", NULL};
...
char* allocate_more(char* buf, size_t old_size, size_t new_size) {
...
readfd(int fd) {
  ...
  size_t pos = 0;
  for (;;) {
    size_t delta = read(fd, buf + pos, n - pos);
    if (delta == 0) break;
    pos += delta;
    if (pos == n) {
      buf = allocate_more(buf, n, n * 2);
      n *= 2;
    }
  }
  if (pos == n) {
    buf = allocate_more(buf, n, n + 1);
    n++;
  }
  buf[pos++] = 0;
  close(fd);
...
char*
runcmd(char* cmd[], const char* begin, const char* end) {
  int pstdin[2];
  int pstdout[2];
  int pstderr[2];
  if (pipe(pstdin) != 0) eprint("Error creating pipe.");
  if (pipe(pstdout) != 0) eprint("Error creating pipe.");
  if (pipe(pstderr) != 0) eprint("Error creating pipe.");
  pid_t cpid = fork();
  if (cpid == -1) eprint("Error forking child.");
  if (cpid == 0) {
    close(pstdin[1]);
    dup2(pstdin[0], 0);
    close(pstdout[0]);
  ...
  while (len > 0) {
    size_t written;
    if ((written = write(pstdin[1], begin, len)) < 0) {
      eprint("Couldn't write to pipe: %s.", strerror(errno));
    }
    len -= written;
  }
  close(pstdin[1]);
  ...
  if (status == 0) {
    close(pstderr[0]);
    return readfd(pstdout[0]);
  } else {
    close(pstdout[0]);
    eprint("Plugin %s exited with %d:\n%s", cmd[0], status, readfd(pstderr[0]));
    return NULL;
  }
}

int
doplugin(const char *begin, const char *end, int newblock) {
  unsigned int i, l_b, l_a;
  const char *p, *start, *stop;

  for(i = 0; i < LENGTH(plugin); i++) {
    l_b = strlen(plugin[i].search_before);
    l_a = strlen(plugin[i].search_after);
    if(end - begin < l_b + l_a || strncmp(begin, plugin[i].search_before, l_b) != 0)
      continue;
    start = begin + l_b;
    p = start - 1;
    do {
      stop = p;
      p = strstr(p + 1, plugin[i].search_after);
    } while(p && p[-1] == '\\');
    if (p && p[-1] != '\\')
      stop = p;
    if(!stop || stop < start || stop >= end)
    ...
    char* out = runcmd(plugin[i].cmd, start, stop);
    size_t len = strlen(out);
    hprint(out, out + len);
    free(out);

    return stop - begin + l_a;
  }
  return 0;
}
